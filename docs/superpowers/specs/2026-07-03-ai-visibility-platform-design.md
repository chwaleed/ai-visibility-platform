...
| Real data | **DataForSEO** — `/v3/keywords_data/google_ads/search_volume/live/` (volume) + `/v3/dataforseo_labs/google/bulk_keyword_difficulty/live/` (difficulty 0–100). Basic auth. One **batched** call each per run | Brief requires real volume/competition data. $50 free trial, no card |
