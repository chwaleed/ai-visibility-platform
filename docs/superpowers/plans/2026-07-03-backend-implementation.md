...
7. **Real data** — DataForSEO endpoints used, batching (2 calls/run), graceful degradation policy; note on DataForSEO's LLM Mentions API as the production upgrade path for cross-model visibility checks.
